...
def add_jobs_to_scheduler(jobs, scheduler):
    for job in jobs:
        if job['command'] == 'play':
            scheduler.add_job(play, 'cron', hour=job['hour'], minute=job['minute'], day_of_week=job['day_of_week'], args=[job['audio_index']])
        if 'video_index' in job:
            scheduler.add_job(play_video, 'cron', hour=job['hour'], minute=job['minute'], day_of_week=job['day_of_week'], args=[job['video_index']])
